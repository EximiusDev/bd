...
EXECUTE CambiarDomicilio 'Ringer', 'Colon 444';
EXECUTE CambiarDomicilio3 'Ringer', 'Colon 444';
 ...
 */
	...
	SELECT CambiarDomicilio3 ('Ringer', 'Colon 444');
...
select * FROM authors WHERE au_lname = 'Yokomoto';
...
CREATE PROCEDURE CambiarDomicilio5 
   ...
   AS 
	  ...
	  PRINT'apellido: ' + @apellido + ', domicilio:' + @domicilio;


EXECUTE CambiarDomicilio5 'Yokomoto', '3 Silver Ct.';
...
CREATE or replace FUNCTION CambiarDomicilio15  
   ( 
    IN prmAu_lname VARCHAR(40), 
    IN prmAddress VARCHAR(40) 
   ) 
  ...
  BEGIN 
      ...
      SELECT prmAddress INTO domicilio;   	-- asignar valores a variable (con SELECT)

      UPDATE authors 
         SET address = domicilio 
         WHERE au_lname LIKE apellido; 
	
	  RAISE NOTICE'apellido: %, domicilio: %', apellido, domicilio;
	  RETURN;  
   END; 
   $$;   

SELECT CambiarDomicilio15 ( 
                     'Yokomoto',      --prmAu_lname 
                     '3 Silver Ct.'   --prmAddress 
                         ); 
...
ALTER PROCEDURE buscarPrecio3 
   ...
   AS 
      ...
      PRINT'PrecUnit: '+ CAST(@PrecUnit AS VARCHAR);
      set @PrecUnit = 999;
 		PRINT'PrecUnit:'+ CONVERT(VARCHAR, @PrecUnit);
      RETURN
      
      
EXECUTE buscarPrecio3
@CodProd = 10,
@PrecUnit = 100

DECLARE @PrecUnit1 float
EXECUTE buscarPrecio3 10, @PrecUnit1 OUTPUT
SELECT @PrecUnit1;
...
CREATE OR REPLACE FUNCTION PublicacionesPorType1 
   ...
   BEGIN 
      ...
      pattern := '%' || prmType || '%'; 				-- Defino pattern
      RAISE NOTICE 'El patrón a buscar es %', pattern; 
     
      SELECT COUNT(*) INTO cantidad 				-- Defino cantidad
         FROM Titles 
 ...
 IN valor NUMERIC    
   ...
   BEGIN 
      ...
      IF precio < valor THEN 
         RETURN 'El precio es menor que ' || valor::VARCHAR; -- cast(valor AS VARCHAR);
      ELSE     
         RETURN 'El precio es mayor que %',  CAST(valor AS VARCHAR); 
      END IF; 
   END; 
   $$  
   LANGUAGE plpgsql 
...
ALTER PROCEDURE CambiarDomicilio  
   ...
   AS 
      ...
      Ej:
 */
ALTER PROCEDURE CambiarDomicilio5 -- ALTER
   ( 
    @prmAu_lname VARCHAR(40), 
    @prmAddress VARCHAR(40) 
   ) 
